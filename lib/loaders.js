...
loaders.model.association.plural.belongsToMany.put = (router, base, model, association, options) => {
  ...
  const get = `get${capitalizeFirstLetter(plural)}`
  const set = `set${capitalizeFirstLetter(plural)}`
  ...
  router.put(`${base}/:associationId`,
    ...
    function * (next) {
      ...
      request.body.id = this.params.associationId
    ...
    function * (next) {
      ...
      yield params.id[set](response.body)

      const data = 
        yield params.id[get]({ id: this.params.associationId })
