...
loaders.model.association.plural.belongsToMany.put = (router, base, model, association, options) => {
  ...
  router.put(base, 
    ...
    function * (next) {
      ...
      let id = switchByType(request.body, {
        object : (data) => data[otherKey],
        array  : (data) => data.map(row => row[otherKey])
      })

      if (!id) {
        this.throw('RestQL: upsert error', 500)
      }
  ...
  router.put(`${base}/:associationId`,
    ...
    function * (next) {
      ...
      let data = 
        yield association.target.findById(associationId)

      if (data) {
        let where = {}
        where[foreignKey] = this.params.id
        where[otherKey]   = associationId

        data = yield through.model.find({ where })

        if (!data)
          this.throw('RestQL: ${model.name} ${association.as} ${associationId} is not found', 404)
      }
