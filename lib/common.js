...
const unionAttributes = (_attributes, attributes) => {
  ...
  let attrs, include = true;

  if (typeof _attributes === 'object') {
    if (_attributes.include)
      _attributes = _attributes.include;
    if (_attributes.exclude) {
      include = false; 
      _attributes = _attributes.exclude;
    }
  }
  ...
  attrs = attrs.filter(attr => !!attributes[attr]);

  if (!include)
    attrs = Object.keys(attributes).filter(key => attrs.indexOf(key) === -1);
...
module.exports.parseQuerystring = (querystring, model) => {
  let _order, _where, _attributes
    , _through, _include, _limit, _offset
