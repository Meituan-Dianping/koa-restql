  ...
  return function * (next) {
    ...
    if (!association) {
      ...
      let associationId    = params.associationId
        ...
        , config           = association.options.restql
        , order;

      if (config && config.writeOnly) {
        this.throw(`${model.name}'s association 
            ${getAssociationName(association)} is writeOnly'`, 403);
      } 
  ...
  return function * (next) {
    ...
    if (!association) {
      ...
      if (config && config.readOnly) {
        this.throw(`${model.name}'s association 
            ${getAssociationName(association)} is readOnly'`, 403);
      } 
