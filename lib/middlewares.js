...
const get = (router, { model, base, association }) => {
...
const post = (router, { model, base, association }) => {
  router.post(`${base}`, postListMiddlewareFactory(options));
}

const put = (router, { model, base, association }) => {
  router.put(`${base}/:id`, putSingleMiddlewareFactory(options));
  router.put(`${base}`, putListMiddlewareFactory(options));
}

const delSingleMiddlewareFactory = ({ model, association }) => {
  if (association) {
    return function * (){};
  } else  {
    return function * (){};
...
const delListMiddlewareFactory = ({ model, association }) => {
  if (association) {
    return function * (){};
  } else {
    return function * (){};
...
const del = (router, options) => {
  router.del(`${base}/:id`, delSingleMiddlewareFactory(options));
  router.del(`${base}`, delListMiddlewareFactory(options));
}
...
const put = (method, model, association) => {
  return function * (next) {
    ...
    if (!association) {
      data = yield* _upsert(model, id, body);
    } else {
      data = yield model.findOne({
        where: { id },
        include
      ...
      data = yield* _upsert(association.target, associationId, body);
    }

        data = yield _create(model);
      } else {

        data = yield _create(body);
      }
    } else {
      let associationId = params.associationId;
