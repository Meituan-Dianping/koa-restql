...
const setup = () => {
  return function * (next) {
    ...
    this.restql.parsedQuery = this.restql.parsedQuery || {}
...
const _findByParams = paramName => (model, query) => {
  return function * (next) {

    query = query || this.restql.parsedQuery || {}
...
const createOrUpdateById = (model) => {
  return function * (next) {
    ...
    if (id === undefined) {
      ...
      data = 
        yield* _create.call(this, model, this.request.body) 
