...
const findByParams = paramName => model => {
  return function * (next) {
...
const getInstanceValidUniqueIndexes = (model, data) => {
...
const handleSequelizeUniqueConstraintError = function * (model, data, error) {
  
  const message = `${model.name} unique constraint error`
  const status  = 409

  const fields       = getInstanceValidUniqueIndexFields(model, data)
...
const _create = function * (model, data, options) {
  ...
  try {
    ...
    data = 
      yield* handleSequelizeUniqueConstraintError.call(this, model, data, e)
...
module.exports.findById            = findByParams('id')
module.exports.findByAssociationId = findByParams('associationId')
