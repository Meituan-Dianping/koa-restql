...
const getUniqueConstraintErrorFields = (model, e) => {
  ...
  fieldsIsValid = Object.keys(e.fields).every(key => attributes[key]);
  ...
  if (e.fields.PRIMARY) {
    ...
    let keys = Object.keys(primaryKeys);
    if (!keys.length)
      return;
    ...
    index = 
      indexes.find(index => e.fields[index.name] && index.unique);
  }
  ...
  return function * (next) {
    ...
    if (!association) {
      ...
      if (!isBelongsToMany) {
        ...
        if (Array.isArray(body)) {
          body.forEach(row => row[foreignKey] = id)
        } else {
          body[foreignKey] = id
        }
      }
